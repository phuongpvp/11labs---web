...
if ($method === 'GET') {
    ...
    try {
        $db = getDB();
        $stmt = $db->prepare("SELECT status, source_file, result_file, duration, points_used, error_message FROM isolation_jobs WHERE id = ?");
        $stmt->execute([$jobId]);
        $job = $stmt->fetch();
        ...
        if ($job['status'] === 'completed') {
            $job['result_url'] = PHP_BACKEND_URL . '/api/results/isolator/' . $job['result_file'];
        }
...
if ($method === 'POST') {
    ...
    if (!verifyWorkerSecret($workerSecret)) { // Use the same secret as other APIs
    ...
    try {
        ...
        switch ($action) {
            case 'get_pending':
                $stmt = $db->query("SELECT id, source_file FROM isolation_jobs WHERE status = 'pending' ORDER BY created_at ASC LIMIT 5");
            ...
            case 'start':
                ...
                $stmt = $db->prepare("UPDATE isolation_jobs SET status = 'processing', worker_uuid = ? WHERE id = ? AND status = 'pending'");
            ...
            case 'charge':
                ...
                $db->beginTransaction();
                $stmt = $db->prepare("SELECT j.user_id, u.quota_used, u.quota_total FROM isolation_jobs j JOIN users u ON j.user_id = u.id WHERE j.id = ? FOR UPDATE");
                $stmt->execute([$jobId]);
                $user = $stmt->fetch();
                ...
                $db->prepare("UPDATE isolation_jobs SET duration = ?, points_used = ? WHERE id = ?")->execute([$duration, $pointsNeeded, $jobId]);
                $db->commit();
            ...
            case 'complete':
                ...
                // Try to create the column safely just in case it doesn't exist
                try {
                    $db->exec("ALTER TABLE isolation_jobs ADD COLUMN api_key_ids VARCHAR(255) DEFAULT ''");
                } catch (\Exception $e) {
                }

                $stmt = $db->prepare("UPDATE isolation_jobs SET status = 'completed', result_file = ?, api_key_ids = ? WHERE id = ?");
            ...
            case 'fail':
                ...
                $stmt = $db->prepare("UPDATE isolation_jobs SET status = 'failed', error_message = ? WHERE id = ?");
